...
def text_segments(input_text: str) -> tuple[list[str], list[list[str]]]:
    # Cannot use '|' character in the text. This is what audio engine uses to
    # split text
    ...
    for section in text.split(SECTION_CHAR):
        for paragraph in section.split(PARAGRAPH_CHAR):
            # format paragraph text for audio
    ...
    # Makes it easier to add audio to screenshot backgrounds_old
    html_template, html_templates, html_template_size = "", [], 0
    audio_clip, audio_clips = "<speak>", []
    for idx, segment in enumerate(tts_segments):
        ...
        # add HTML to template and add audio to audio clip
            ...
            audio_clip += format_ssml(segment)
        # create new HTML template and add to list of templates
        else:
            html_templates.append(f"{html_template}</p>")
            html_template = f"<p>{format_reddit_markdown(segment)}"
            html_template_size = len(segment)
            audio_clips.append(f"{audio_clip}</speak>")
            audio_clip = f"<speak>{format_ssml(segment)}"
...
def text_segments_old(input_text: str) -> tuple[list[str], list[list[str]]]:
    # Cannot use '|' character in the text. This is what audio engine uses to
    # split text
...
def format_ssml(text):
    ...
    clean_text = re.sub(bad_chars, "", clean_text)
    ...
    clean_text = clean_text.replace(".", '. <break time="0.25s"/>')
    clean_text = clean_text.replace(":", ': <break time="0.25s"/>')
    return clean_text
...
def title_flair(title_flair) -> None:
    ...
    if not title_flair or title_flair != "Series":
        return ""
    if title_flair != "Series":
        f'<span class="linkflairlabel">Mature Content</span>'
    return f'<span class="linkflairlabel">{title_flair}</span>'
...
def extract_reddit_ids(text: str) -> set[str]:
    ...
    text_ids = set()
    matches = re.findall("r/nosleep/comments/([^/]+)", text)
    if matches:
        for match in matches:
            text_ids.add(match)

    matches = re.findall(r"https://redd.it/([^)]+)", text)
    if matches:
        for match in matches:
            text_ids.add(match)
