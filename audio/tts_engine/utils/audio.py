...
def load_wav_to_torch(full_path):
    ...
    return (
        torch.FloatTensor(data.astype(np.float32)) / norm_fix,
        sampling_rate,
    )
...
def combine_audio(clip, path, num):
    print(f"Combining audio segments into: {clip}.wav")
    full_path = f"{path}/{clip}"
    final = [load_audio(f"{full_path}/{s}.wav", 24000) for s in range(num)]
...
def normalize_tacotron_mel(mel):
    return (
        2 * ((mel - TACOTRON_MEL_MIN) / (TACOTRON_MEL_MAX - TACOTRON_MEL_MIN))
        - 1
    )
