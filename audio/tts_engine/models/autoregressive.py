...
from .arch_util import AttentionBlock
from ..utils.typical_sampling import TypicalLogitsWarper


def null_position_embeddings(range, dim):
    return torch.zeros(
        (range.shape[0], range.shape[1], dim), device=range.device
    )
    ...
    def __init__(self, channels, mel_channels=80, resblocks_per_reduction=2):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Sequential(
                *[
                    ResBlock(channels // 4)
                    for _ in range(resblocks_per_reduction)
                ]
            ),
            nn.Conv1d(
                channels // 4, channels // 2, kernel_size=3, stride=2, padding=1
            ),
            ...
            nn.Conv1d(
                channels // 2, channels, kernel_size=3, stride=2, padding=1
            ),
...
class UnifiedVoice(nn.Module):
    def __init__(
        ...
        self.start_text_token = (
            ...
            if start_text_token is None
            else start_text_token
        )
        ...
        self.text_head = nn.Linear(
            model_dim, self.number_text_tokens * types + 1
        )
