...
from .arch_util import normalization, AttentionBlock
...
def is_sequence(t):
    return t.dtype == torch.longclear
...
class DiffusionTts(nn.Module):
    ...
    def timestep_independent(
        ...
        if is_latent(aligned_conditioning):
            ...
            code_emb = self.code_embedding(aligned_conditioning).permute(
                0, 2, 1
            )
    ...
    def forward(
        ...
        for i, lyr in enumerate(self.layers):
            ...
            if (
                ...
                with autocast(
                    x.device.type, enabled=self.enable_fp16 and i != 0
                ):
                    x = lyr(x, time_emb)
